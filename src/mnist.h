...
void load_mnist()
{
	train_image = malloc(NUM_TRAIN * sizeof(double*));
	for (int i = 0; i < NUM_TRAIN; i++) {
		train_image[i] = malloc(SIZE * sizeof(double));
	}

	test_image = malloc(NUM_TEST * sizeof(double*));
	for (int i = 0; i < NUM_TEST; i++) {
		test_image[i] = malloc(SIZE * sizeof(double));
	}

	train_label = malloc(NUM_TRAIN * sizeof(int));
	test_label = malloc(NUM_TEST * sizeof(int));
